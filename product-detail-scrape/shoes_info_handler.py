...
def scrape0(event, context):
  data = fetch_url('shoes_url_0.csv')
  info = get_info_all(data)
  file_name = f"shoes_info_0.csv"
  save_file_to_s3('ssense-mens-product-scrape', info, file_name)
  
  return {'status': 'success'}

def scrape1(event, context):
  data = fetch_url('shoes_url_1.csv')
  info = get_info_all(data)
  file_name = f"shoes_info_1.csv"
  save_file_to_s3('ssense-mens-product-scrape', info, file_name)
  
  return {'status': 'success'}

def scrape2(event, context):
  data = fetch_url('shoes_url_2.csv')
  info = get_info_all(data)
  file_name = f"shoes_info_2.csv"
  save_file_to_s3('ssense-mens-product-scrape', info, file_name)
